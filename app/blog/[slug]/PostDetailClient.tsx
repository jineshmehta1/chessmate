...
export default function PostDetailClient({ post }: { post: any }) {
  // Helper to safely render the content structure
          ...
          <div className="space-y-12 font-medium">
             ...
             {content.sections?.map((section: any, idx: number) => (
               <div key={idx} className="space-y-4">
                  <h2 className="text-2xl md:text-4xl text-black border-b-4 border-black inline-block pb-1">{section.heading}</h2>
               ...
               <div className="space-y-8">
                  {content.benefits.map((benefit: any) => (
                    <div key={benefit.number} className="flex gap-6 group">
                      ...
                      <div>
                        <h3 className="text-xl font-black uppercase text-black mb-2">{benefit.title}</h3>
                        <p>{benefit.body}</p>
                      </div>
                    </div>
                  ))}
             ...
             {/* Dynamic Steps/Phases (Improve Rating / Tournament Prep) */}
             {(content.steps || content.phases)?.map((item: any, idx: number) => (
               <div key={idx} className="p-8 border-4 border-black shadow-[8px_8px_0px_0px_black] bg-white">
                  ...
                  <h3 className="text-2xl font-[1000] uppercase text-black mb-4">{item.title || item.name}</h3>
                  <p>{item.body}</p>
                  {item.points && (
             ...
             {/* Elite Insight Callout (Maintains your Brutalist design) */}
             <div className="bg-black text-white p-8 md:p-12 border-4 border-black shadow-[12px_12px_0px_0px_rgba(234,179,8,1)] my-16 relative overflow-hidden group">
                ...
                <h4 className="text-xl md:text-2xl font-[1000] uppercase italic mb-4 tracking-tighter">FIDE Master Insight</h4>
                <p className="text-gray-300 text-base md:text-lg font-medium italic leading-relaxed max-w-2xl">
                  {content.conclusion?.slice(0, 150)}...
                </p>
             ...
             <div className="bg-[#EAB308]/10 p-8 border-2 border-dashed border-[#EAB308] rounded-xl">
               <h3 className="text-black font-black uppercase mb-4 tracking-widest flex items-center gap-2">
                 The Bottom Line
               </h3>
